...
import { Home, Users, TrendingUp, FileText, DollarSign, Settings } from "lucide-react";
import { NavLink } from "react-router-dom";

export default function ZonalSiderbar() {
  return (
    <aside className="h-screen w-72 bg-gradient-to-b from-slate-950 via-emerald-950 to-slate-900 text-white relative overflow-hidden">
      ...
      <div className="relative px-8 py-6 border-b border-white/10">
        ...
        <p className="text-xs text-slate-400 mt-1">Zonal Manager</p>
      </div>

      {/* Navigation */}
      <nav className="relative mt-6 px-4 space-y-2">
        <NavItem to="/dashboard/zonalmanager" icon={<Home size={20} />} label="Dashboard" />
        <NavItem to="/dashboard/zonalmanager/employees" icon={<Users size={20} />} label="Area Employees" />
        <NavItem to="/dashboard/zonalmanager/performance" icon={<TrendingUp size={20} />} label="Performance" />
