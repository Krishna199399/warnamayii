...
import { Home, Users, TrendingUp, CheckSquare, FileText, DollarSign, Settings } from "lucide-react";
import { NavLink } from "react-router-dom";

export default function DoSidebar() {
  return (
    <aside className="h-screen w-72 bg-gradient-to-b from-slate-950 via-emerald-950 to-slate-900 text-white relative overflow-hidden">
      ...
      <div className="relative px-8 py-6 border-b border-white/10">
        ...
        <p className="text-xs text-slate-400 mt-1">DO Manager</p>
      </div>

      {/* Navigation */}
      <nav className="relative mt-6 px-4 space-y-2">
        <NavItem to="/dashboard/domanager" icon={<Home size={20} />} label="Dashboard" />
        <NavItem to="/dashboard/domanager/advisors" icon={<Users size={20} />} label="Advisors" />
        <NavItem to="/dashboard/domanager/performance" icon={<TrendingUp size={20} />} label="Performance" />
        <NavItem to="/dashboard/domanager/tasks" icon={<CheckSquare size={20} />} label="Tasks" />
